...
mkdir -p "$FAKE_BIN"
# Create mock environment where hostname is explicitly missing
NEW_PATH="$FAKE_BIN:/usr/bin:/bin"
...
SWARM_NODES="$HOME/.config/knot/swarms/home/nodes"
mkdir -p "$SWARM_NODES"
...
export KNOT_ACTIVE_SWARM="home"
ANCHOR_ID="desktop" ANCHOR_HOST="kuasha-z490ud"
...
grep -q "Host desktop.home" "$ssh_cfg" || { echo "Missing 'Host desktop.home' in ~/.ssh/config" >&2; exit 1; }
